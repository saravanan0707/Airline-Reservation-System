...
Public Class PlaceNameForm
    ...
    Sub SaveRecord()
        ...
        Conn.Open()
        Dim Cmd0 As New SqlCommand("select PlaceName from PlaceTab where PlaceName='" & UCase(TextBox1.Text) & "'", Conn)
        ...
        If Conn.State = ConnectionState.Open Then Conn.Close()
        Conn.Open()
        q1Var = "insert into PlaceTab("
        q2Var = " values("
        q1Var = q1Var & "PlaceName" & ","
        q2Var = q2Var & "'" & UCase(TextBox1.Text) & "',"
        q1Var = q1Var & "Details" & ","
        q2Var = q2Var & "'" & TextBox2.Text & "',"
        q1Var = q1Var & "placeType" & ")"
    ...
    Sub disRecords()
        ...
        Dim adp As New SqlDataAdapter("Select * From PlaceTab order by PlaceName", Conn)
        adp.Fill(DS1)
    ...
    Private Sub DG1_CellContentClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles DG1.CellContentClick
        ...
        If Conn.State = ConnectionState.Open Then Conn.Close()
        Conn.Open()
        Dim Cmd0 As New SqlCommand("select * from PlaceTab where PlaceName='" & pkVar & "'", Conn)
        Dim D1 As SqlDataReader = Cmd0.ExecuteReader()
        If D1.HasRows Then
            D1.Read()
    ...
    Private Sub butModify_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles butModify.Click
        ...
        If Conn.State = ConnectionState.Open Then Conn.Close()
        Conn.Open()
        Dim cmd1 As New SqlCommand("Delete from PlaceTab where PlaceName='" & pkVar & "'", Conn)
        cmd1.ExecuteNonQuery()
        If Conn.State = ConnectionState.Open Then Conn.Close()
        SaveRecord()
        butNew.Enabled = True
